...
class ActionController(object):
    ...
    current action that the object is performing."""
        ...
        time_into_action = time - self.last_action_changeover
        self.owner.bone_transforms = self.current_animation().get_bone_transforms(
            time_into_action, True
        )
        diff = (
            self.current_animation().get_root_offset(time_into_action)
            - self.current_animation().get_root_offset(0)
        ) * self.owner.scale
        r = self.owner.angle
        # fmt: off
