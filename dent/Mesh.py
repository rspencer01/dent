...
def get_texture_filename(material, texture_type, directory):
  ...
  if texture_type in dent_formats:
    filename = os.path.join(
        directory,
        dent_formats[texture_type].format(material_name))
  ...
  if ('file', texture_type) in material.properties:
    filename = os.path.join(
        directory,
        material.properties[('file', texture_type)])
    if os.path.exists(filename):
...
class Mesh(object):
  ...
  def load_from_assimp(self, assimp_mesh, directory, scene, parent):
    ...
    self.diffuse_texture_file = get_texture_filename(assimp_mesh.material, pyassimp.material.aiTextureType_DIFFUSE, directory)
